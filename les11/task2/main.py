...
def save_file(data):
    with open ('./les11/task2/Phonebook.json', 'w') as f:
        json.dump(data, f, indent=2)

def delete(isUpdate):
    phone = input('What number we need to find: ')
    f =  open ('./les11/task2/Phonebook.json', 'r')
...
def add_entry():
    ...
    with open ('./les11/task2/Phonebook.json', 'r') as f:
        newJ = json.load(f)
        newJ.append(data)
    save_file(newJ)

def app_mainscreen():
    if path.isfile('./les11/task2/Phonebook.json') is False:
